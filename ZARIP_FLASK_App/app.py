...
class AIAssistant:
    ...
    def answer_question(question, results=None):
        ...
        if any(word in question_lower for word in ['var', 'value at risk', 'value-at-risk']):
            return AI_KNOWLEDGE['var']
        elif any(word in question_lower for word in ['cvar', 'conditional var', 'expected shortfall']):
...
def calculate_metrics(portfolio_losses, regional_payouts, params):
    ...
    total_state_subsidy = sum(REGIONS[name]['exposure_ha'] * params['sum_insured'] * REGIONS[name]['subsidy_rate'] for name in REGION_NAMES)
    total_farmer_contribution = loaded_premium - total_state_subsidy
    cfl_95 = max(0, var_95 - loaded_premium)
    cfl_99 = max(0, var_99 - loaded_premium)
    fiscal_saving = 53.5e6
...
def generate_charts(results):
    ...
    colors = ['#2ecc71' if s < 50 else '#f39c12' if s < 70 else '#e74c3c' for s in subsidies]
    
    bars = ax3.bar(regions, subsidies, color=colors, alpha=0.8, edgecolor='white', linewidth=2)
    ax3.set_xlabel('Region', fontsize=12)
    ax3.set_ylabel('Subsidy Rate (%)', fontsize=12)
    ax3.set_title('Smart Subsidy Allocation (Tiered by Vulnerability)', fontsize=14, fontweight='bold')
    ax3.axhline(y=50, color='#7f8c8d', linestyle='--', alpha=0.7, label='50% Reference')
    ax3.legend()
    ax3.grid(True, alpha=0.3, axis='y')
    
    for bar, s in zip(bars, subsidies):
        ax3.annotate(f'{s:.0f}%', (bar.get_x() + bar.get_width()/2, bar.get_height()),
